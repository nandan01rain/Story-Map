  ...
  <nav>
    ...
    <details class="dd"><summary>?</summary><div class="pop" id="how">
      <ul>
        ...
        <li>drag to orbit freely, shift-drag to pan, scroll to zoom</li>
        <li>on touch: one finger orbits, two fingers pinch to zoom and drag to pan</li>
        <li>the &#9633; button returns to the canonical side-on view</li>
      </ul>
    </div></details>
  </nav>
</div>
...
// theta is ELEVATION above the spine, phi is AZIMUTH around it. A true spherical offset, so
// dist is really the distance; an earlier version scaled y and z differently and put the
// camera a third of the way in from where it claimed to be.
//
// phi was fixed at 0 until 2026-08-30, on the reasoning that X keeping a fixed screen
// direction is what makes the picture learnable. Freed by decision: a saga seen only from
// one side hides the interlacing that is the entire point of drawing it as a braid, and the
// reset control is one tap away for anyone who wants the canonical view back.
let theta = 0.34, phi = 0, dist = 480;
function updateCamera() {
  const flat = Math.cos(theta) * dist;
  camera.position.set(
    target.x + Math.sin(phi) * flat,
    target.y + Math.sin(theta) * dist,
    target.z + Math.cos(phi) * flat,
  );
  camera.up.set(0, 1, 0);          // still no ROLL: the horizon stays level at any angle
...
// Panning has to follow the CAMERA's axes now, not the world's. While the view was locked to
// one side, "drag right" and "move along +X" were the same thing; once the camera can be
// anywhere, moving the target along world X sends the picture sideways at an angle to the
// gesture, which reads as a broken control rather than a rotated one.
const panRight = new THREE.Vector3();
const panUp = new THREE.Vector3();
function panBy(from, dx, dy) {
  camera.matrixWorld.extractBasis(panRight, panUp, new THREE.Vector3());
  const k = dist * 0.0016;
  target.set(from.tx, from.ty, from.tz)
    .addScaledVector(panRight, -dx * k)
    .addScaledVector(panUp, dy * k);
}
...
renderer.domElement.addEventListener('pointerdown', (e) => {
  ...
  if (pointers.size === 2) {
    ...
    //
    // Two fingers do BOTH jobs at once -- the gap zooms, the centroid pans -- which is what
    // a phone needs now that a one-finger drag orbits instead of panning. Anything else
    // would leave no way to travel along the saga on touch.
    drag = null;
    const c = pinchCentroid();
    pinch = { gap: Math.max(1, pinchGap()), dist, cx: c.x, cy: c.y,
              tx: target.x, ty: target.y, tz: target.z };
    return;
  }
  if (pointers.size > 2) return;
  drag = { x: e.clientX, y: e.clientY, theta, phi, tx: target.x, ty: target.y,
           tz: target.z, moved: false, pan: e.button === 2 || e.shiftKey };
...
renderer.domElement.addEventListener('pointermove', (e) => {
  ...
  if (pinch && pointers.size >= 2) {
    ...
    const c = pinchCentroid();
    panBy(pinch, c.x - pinch.cx, c.y - pinch.cy);
    updateCamera();
    return;
  }
  if (!drag) return;
  ...
  if (drag.pan) {
    ...
    // Sensitivity relative to the viewport, not a fixed rate per pixel: at a fixed rate the
    // full elevation range needs ~450px of vertical drag, which a phone in landscape does
    // not have, so the arc ran out under the thumb. A full-height drag now covers it
    // whatever the screen, and a full-width drag turns the braid roughly once around.
    const perPxY = 2.9 / Math.max(320, innerHeight);
    const perPxX = 6.0 / Math.max(320, innerWidth);
    theta = Math.max(-1.35, Math.min(1.35, drag.theta - dy * perPxY));
    phi = drag.phi + dx * perPxX;
  }
  updateCamera();
});
...
document.getElementById('zoom-fit').addEventListener('click', () => {
  theta = 0.34; phi = 0; dist = framingDistance(); target.set(0, 0, 0); updateCamera();
});
...
document.getElementById('top').addEventListener('click', () => {
  theta = 1.3; phi = 0; dist = framingDistance() * 0.85; target.set(0, 0, 0); updateCamera();
});
