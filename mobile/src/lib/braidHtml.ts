...
function applyEmphasis(lit, strength) {
  ...
  ribbonObjs.forEach((g) => {
    ...
    if (!active) {
      ...
      mat.emissiveIntensity = 0.22 + 0.5 * strength;
      mat.transparent = false;
      mat.opacity = 1;
    } else {
      mat.color.copy(saturated(base, -0.45, 0.72));
      mat.emissive.set(0x000000);
      mat.emissiveIntensity = 0;
      mat.transparent = true;
      mat.opacity = 1 - 0.72 * strength;
    }
    ...
    g.userData.glows.forEach((gl) => {
      ...
      gl.material.opacity = !active ? rest
        : (on ? Math.min(0.6, rest * (1 + 2.6 * strength)) : rest * (1 - 0.85 * strength));
    });
  });
}
